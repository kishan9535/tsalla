      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        >
          {teamStats.map((stat, index) => (
            <div
              key={stat.label}
              className="bg-black p-6 rounded-xl border border-gray-800 hover:border-blue-500 transition-all duration-300 text-center group"
            >
              <stat.icon className="w-8 h-8 text-blue-400 mx-auto mb-4 group-hover:text-blue-300 transition-colors" />
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredMembers.map((member, index) => (
            <div
              key={member.name}
              className={`bg-black rounded-2xl overflow-hidden border border-gray-800 hover:border-blue-500 transition-all duration-500 group ${
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
              }`}
            ...
            >
              ...
              <div className="relative h-64 overflow-hidden">
                ...
                <div className="absolute top-4 right-4">
                  <span className="bg-blue-600 bg-opacity-90 backdrop-blur-sm text-white px-3 py-1 rounded-full text-sm font-semibold">
                    {member.department}
                  </span>
                </div>
              </div>
